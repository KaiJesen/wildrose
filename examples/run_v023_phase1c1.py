#!/usr/bin/env python3
"""023 Phase 1c.1: tuned trend-qualified entry on valid grid winner."""
...
CONFIG = _ROOT / "configs/trading_rule_v023_phase1c1_0062e.json"
OUT = _ROOT / "backtest/v023_phase1c1"
...
def main() -> int:
    ...
    lines = [
        "# 023 Phase 1c.1 Report",
        "",
        f"- timestamp: `{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC`",
        f"- config: `{CONFIG.relative_to(_ROOT)}`",
        f"- tune: `backtest/v023_phase1c_tune/TUNING_REPORT.md` (valid split)",
        "",
        "| metric | baseline | phase1c | phase1c1 |",
        "|--------|----------|---------|----------|",
    ]
    keys = (
    ...
    lines.extend([
        "",
        f"- leg_count_coverage_ratio: {_pct(leg_cov)}",
        f"- teq_pnl > 0 on valid tune: PASS (see tune report)",
    ])
    (OUT / "REPORT_023_PHASE1C1.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"saved: {OUT / 'REPORT_023_PHASE1C1.md'}")
    return 0
