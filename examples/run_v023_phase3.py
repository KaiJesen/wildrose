#!/usr/bin/env python3
"""023 Phase 3: regime-aware dynamic thresholds on phase2 baseline."""
...
CONFIG = _ROOT / "configs/trading_rule_v023_phase3_0062e.json"
OUT = _ROOT / "backtest/v023_phase3"
BASELINE = _ROOT / "backtest/v023_baseline/participation_metrics.json"
PHASE2 = _ROOT / "backtest/v023_phase2/participation_metrics.json"
...
def main() -> int:
    ...
    p2 = json.loads(PHASE2.read_text()) if PHASE2.is_file() else {}
    ...
    b2 = p2.get("test", {}).get("runner_metrics", {})
    ...
    lines = [
        ...
        "",
        "| metric | baseline | phase2 | phase3 |",
        "|--------|----------|--------|--------|",
    ]
    keys = (
        "total_return", "max_drawdown", "trade_count",
        "trend_qualified_open_count", "trend_qualified_pnl",
        "leg_coverage_ratio", "short_trend_capture_ratio", "long_trend_capture_ratio",
    )
    for k in keys:
        a = float(bm.get(k, 0))
        b = float(b2.get(k, 0))
    ...
    (OUT / "REPORT_023_PHASE3.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"saved: {OUT / 'REPORT_023_PHASE3.md'}")
    return 0
