#!/usr/bin/env python3
"""024 Phase 0: build hindsight leg-participation labels (training-only)."""
...
DEFAULT_OUT = "data/labels/leg_participation"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build 024 leg participation labels")
    add_data_args(p)
    add_feature_args(p)
    add_segment_args(p)
    ...
    p.add_argument("--mae-atr-limit", type=float, default=1.5)
    p.add_argument("--max-entry-progress", type=float, default=0.35)
...
def main() -> int:
    ...
    label_cfg = LegParticipationLabelConfig(
        ...
        mae_atr_limit=args.mae_atr_limit,
        max_entry_progress=args.max_entry_progress,
    )
    ...
    for split in splits:
        print(f"building labels: split={split}")
        ...
        print(
            f"  bars={int(s['bar_count'])} legs={int(s['confirmed_leg_count'])} "
            f"ideal_long_rate={s['ideal_participate_long_rate']:.4f}"
        )
