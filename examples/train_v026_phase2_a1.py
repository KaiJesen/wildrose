#!/usr/bin/env python3
"""026 Phase 2 A1: three-tier CORAL labels on C3+C1+D1 stack."""
...
INIT_CKPT = _ROOT / "checkpoints/026_phase1_c1d1/market_state_best.pt"
PARTICIPATION_TIERS = 3


def _load_init_checkpoint(model, ckpt_path: Path, device: torch.device) -> None:
    ...
    skipped = sorted(set(ck["model"]) - set(filtered))
    if skipped:
        coral_skipped = [k for k in skipped if "participation_attn_" in k and ".out.2." in k]
        if coral_skipped:
            print(f"skipped {len(coral_skipped)} CORAL head tensors (shape change)")
    model.load_state_dict(filtered, strict=False)
    print(f"loaded init checkpoint: {ckpt_path} ({len(filtered)} tensors)")
...
def main() -> int:
    ...
    if not args.labels_dir:
        args.labels_dir = recipe.get("labels_dir", "data/labels/leg_participation_a1")
    args.checkpoint_dir = recipe.get("checkpoint_dir", "checkpoints/026_phase2_a1")
    if not args.report_dir:
        args.report_dir = recipe.get("report_dir", "reports/026_phase2_a1")
    ...
    print(
        f"026 Phase2 A1 CORAL λ_part={part_w} lr={args.lr} tiers={PARTICIPATION_TIERS} "
    ...
    _load_init_checkpoint(model, init_ckpt, device)
    ...
    print(f"training {len(train_params)} A1+C3+C1 tensors (CORAL out re-init)")
    ...
    print(
        ...
        f"part_auc_tier1={init_metrics.get('participation_auc_tier1', 0):.4f}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"tier1_auc={valid_m.get('participation_auc_tier1', 0):.4f} "
    ...
    print(
        ...
        f"tier1={valid_m.get('participation_auc_tier1', 0):.4f} "
        f"test part_auc={test_m.get('participation_auc', 0):.4f}"
    )
    return 0
