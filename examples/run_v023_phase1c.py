#!/usr/bin/env python3
"""023 Phase 1c: trend-qualified entry on phase1b baseline."""
...
CONFIG = _ROOT / "configs/trading_rule_v023_phase1c_0062e.json"
OUT = _ROOT / "backtest/v023_phase1c"
BASELINE = _ROOT / "backtest/v023_baseline/participation_metrics.json"
PHASE1B = _ROOT / "backtest/v023_phase1b/participation_metrics.json"
...
def main() -> int:
    ...
    p1b = json.loads(PHASE1B.read_text()) if PHASE1B.is_file() else {}
    test = results.get("test", {}).get("runner_metrics", {})
    part = results.get("test", {}).get("participation", {})
    bm = base.get("test", {}).get("runner_metrics", {})
    b1b = p1b.get("test", {}).get("runner_metrics", {})
    ...
    lines = [
        ...
        "",
        "| metric | baseline | phase1b | phase1c |",
        "|--------|----------|---------|---------|",
    ]
    keys = (
        "total_return", "max_drawdown", "trade_count", "trend_qualified_open_count",
        "trend_qualified_pnl", "leg_coverage_ratio", "short_trend_capture_ratio",
    ...
    (OUT / "REPORT_023_PHASE1C.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"saved: {OUT / 'REPORT_023_PHASE1C.md'}")
    return 0
