#!/usr/bin/env python3
"""Tune v023 Phase 1c trend-entry qualifier params on valid split."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tune v023 phase1c trend entry qualifier")
    add_data_args(p)
    add_feature_args(p)
    add_segment_args(p)
    add_vq_args(p)
    p.add_argument("--checkpoint", default="prod/v0.0.0/checkpoint/market_state_best.pt")
    p.add_argument("--base-config", default="configs/trading_rule_v023_phase1c_0062e.json")
    p.add_argument("--split", choices=["train", "valid", "test"], default="valid")
    p.add_argument("--trunk-layers", type=int, default=2)
    p.add_argument("--device", default="cpu")
    p.add_argument("--output-dir", default="backtest/v023_phase1c_tune")
    p.add_argument("--tuned-config-out", default="configs/trading_rule_v023_phase1c1_0062e.json")
    p.add_argument("--baseline-return", type=float, default=0.0243)
...
def _score(metrics: dict[str, float], *, baseline_return: float) -> float:
    ...
    teq_pnl = float(metrics.get("trend_qualified_pnl", 0.0))
    teq_opens = float(metrics.get("trend_qualified_open_count", 0.0))
    ...
    if teq_pnl < -0.03:
        score -= 1.0
    if total_return < baseline_return * 0.9:
        score -= 0.8
    if max_drawdown < -0.06:
        score -= 0.8
    if trades > 25:
        score -= (trades - 25) * 0.03
    return score
...
def main() -> int:
    ...
    grid = {
        "min_edge_long": [0.0, 0.01, 0.02],
        "min_prob_long": [0.26, 0.28, 0.30],
        "relax_edge_mult": [0.55, 0.65],
        "require_confirmed_leg": [False, True],
        "require_best_point": [False, True],
    }
    keys = list(grid.keys())
    results: list[dict] = []
    ...
    for combo in product(*(grid[k] for k in keys)):
        ...
        teq = replace(
            base_cfg.trend_entry_qualifier,
            min_edge_long=params["min_edge_long"],
            min_prob_long=params["min_prob_long"],
            relax_edge_mult=params["relax_edge_mult"],
            require_confirmed_leg=params["require_confirmed_leg"],
            require_best_point=params["require_best_point"],
        )
        cfg = replace(base_cfg, trend_entry_qualifier=teq)
        metrics = _run_with_cfg(df, cached, start_idx, end_idx, cfg)
        row = {**params, **{k: float(metrics[k]) for k in (
            "total_return", "max_drawdown", "trade_count", "trend_qualified_open_count",
            "trend_qualified_pnl", "leg_coverage_ratio", "long_trend_capture_ratio",
        )}}
        row["score"] = _score(metrics, baseline_return=args.baseline_return)
        results.append(row)
        print(
            f"score={row['score']:.3f} ret={row['total_return']:.4f} "
            f"teq={int(row['trend_qualified_open_count'])} teq_pnl={row['trend_qualified_pnl']:.4f} {params}"
        )
    ...
    payload["trend_entry_qualifier"] = {**payload["trend_entry_qualifier"], **{k: best[k] for k in keys}}
    payload["_023_meta"] = {
        "phase": "1c1",
        "recipe": "trend_qualified_entry_tuned",
        "source_phase1c": args.base_config,
    ...
    lines = ["# v023 Phase 1c tune", "", f"split: {args.split}", "", "## top results", ""]
    for r in results[: args.top_k]:
        lines.append(
            f"- score={r['score']:.3f} ret={r['total_return']:.4f} dd={r['max_drawdown']:.4f} "
            f"teq={int(r['trend_qualified_open_count'])} teq_pnl={r['trend_qualified_pnl']:.4f} "
