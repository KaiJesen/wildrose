...
cp "$ROOT/configs/trading_rule_v021_full_bias_0062e.json" "$PROD/config/trading_rule.json"
cp "$ROOT/prod/v0.0.0/checkpoint/market_state_best.pt" "$PROD/checkpoint/market_state_best.pt"
cp "$ROOT/backtest/backtest_v021_btcusdt_test/metrics.json" "$PROD/metrics/backtest_test_oos.json"
cp "$ROOT/backtest/backtest_v021_btcusdt_test/equity_curve.png" "$PROD/metrics/backtest_plot.png"
cp "$ROOT/backtest/backtest_v021_btcusdt_test/trade_points.png" "$PROD/metrics/backtest_trade_points.png"
cp "$ROOT/backtest/v021_cd_validation/VALIDATION_REPORT.md" "$PROD/docs/REPORT_021_VALIDATION.md"
cp "$ROOT/backtest/v021_cd_validation/validation_summary.json" "$PROD/metrics/validation_summary.json"
...
cat > "$PROD/MANIFEST.json" <<EOF
{
  ...
  "strategy": "v021 trend-bias full_bias",
  ...
  "supplemental_references": {
    "phase_validation": "docs/REPORT_021_VALIDATION.md",
    "validation_summary": "metrics/validation_summary.json",
    "primary_report": "docs/REPORT_021_BACKTEST.md",
