...
def _score_layer(direction: BiasDirection, target: BiasDirection, weight: int) -> int:
    if direction == BiasDirection.NONE:
        return 0
    if direction == target:
        return weight
    return -weight
...
class TrendBiasBuilder:
    ...
    def build(
        ...
        *,
        ...
        raw_long = (
            _score_layer(macro_direction, BiasDirection.UP, 1)
            + _score_layer(leg_direction, BiasDirection.UP, 2)
            + _score_layer(micro_direction, BiasDirection.UP, 1)
        )
        raw_short = (
            _score_layer(macro_direction, BiasDirection.DOWN, 1)
            + _score_layer(leg_direction, BiasDirection.DOWN, 2)
            + _score_layer(micro_direction, BiasDirection.DOWN, 1)
        )
        align_long = _clamp_alignment(raw_long)
        align_short = _clamp_alignment(raw_short)
