...
class ModelSignalProvider:
    ...
    def from_checkpoint(
        ...
        *,
        ...
        slow_up_calibrator = None
        std_trend_calibrator = None
        ...
        if ch_cfg.enabled and ch_cfg.slow_up_gate.enabled:
            ...
            if gate.use_calibrated and gate.calibration_path:
                ...
                if ch_path.is_file():
                    slow_up_calibrator = ChannelEdgeCalibrator.load(ch_path)
        if ch_cfg.enabled and ch_cfg.std_trend_gate.enabled:
            gate = ch_cfg.std_trend_gate
            if gate.use_calibrated and gate.calibration_path:
                ch_path = Path(gate.calibration_path)
                if ch_path.is_file():
                    std_trend_calibrator = ChannelEdgeCalibrator.load(ch_path)
        return cls(
            model=model,
            bars=bars,
    ...
    def _attach_channel_edges(self, sig: TradingSignal) -> None:
        ...
        slow_gate = ch_cfg.slow_up_gate
        if slow_gate.enabled:
            ...
            slow_long, slow_short = apply_channel_calibration(
                slow_long_raw,
                slow_short_raw,
                calibrator=self.slow_up_calibrator,
                use_calibrated=slow_gate.use_calibrated,
            )
            sig.slow_up_edge_long = slow_long
            sig.slow_up_edge_short = slow_short
        std_gate = ch_cfg.std_trend_gate
        if std_gate.enabled:
            std_long_raw, std_short_raw = compute_channel_edge_raw(
                legacy_edge=sig.edge,
                teq_edge_long_raw=teq_long_raw,
                teq_edge_short_raw=teq_short_raw,
                participate_score_long=sig.participate_score_long,
            ...
            if not slow_gate.enabled:
                sig.effective_edge_long_diag = std_long_raw
                sig.effective_edge_short_diag = std_short_raw
            std_long, std_short = apply_channel_calibration(
                std_long_raw,
                std_short_raw,
                calibrator=self.std_trend_calibrator,
                use_calibrated=std_gate.use_calibrated,
            )
            sig.std_trend_edge_long = std_long
            sig.std_trend_edge_short = std_short
